...
const modalSteps = [
    { message: "Generating proofs...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: "Verifying zero-knowledge...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: "Registering nullifier...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: "Identity Verified!", icon: <PartyPopper className="w-12 h-12 text-green-500" /> }
];
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "AlgoKYC", onContinue }: AuthComponentProps) => {
    ...
    const handleFinalSubmit = (e: React.FormEvent) => {
        ...
        if (password !== confirmPassword) {
            ...
            setTimeout(() => {
                ...
                }, 3000); // 3 seconds after success to continue
            }, totalDuration);
        }
    };
    ...
    return (
        ...
        `}</style>
            ...
            <div className={cn("flex w-full flex-1 h-full items-center justify-center bg-transparent", "relative overflow-hidden pt-8")}>
                <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                    <AnimatePresence mode="wait">
                        {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                            <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-5xl tracking-tight text-foreground whitespace-nowrap">Do your KYC</p></div></BlurFade>
                            <BlurFade delay={0.25 * 3}><div className="flex items-center justify-center gap-4 w-full">
                                <GlassButton contentClassName="flex items-center justify-center gap-2" size="sm" onClick={() => {
                                    setAuthStep("password");
                                }}>
                                    <span className="font-semibold text-foreground">Upload Aadhaar XML</span>
                                </GlassButton>
                            </div></BlurFade>
                        </motion.div>}
                        {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                            ...
                            <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">Used to secure your zero-knowledge proof.</p></BlurFade>
                        </motion.div>}
